...
test.describe('File upload and download flow', () => {
  ...
  async function createTestFile(): Promise<{ name: string; mimeType: string; buffer: Buffer }> {
    return {
      name: testFileName,
      mimeType: 'text/plain',
  ...
  async function uploadFile(page: Page, withPassword: boolean = false, password: string = 'test123') {
    ...
    const uploadButton = page.locator('[data-test="upload-button"]').or(mobileNextButton);
    await uploadButton.click();

    await expect(page.locator('input[data-test="download-link-input"]')).toBeVisible({ timeout: 15000 });
  }
  ...
  test('upload file without password and download successfully', async ({ page, context }) => {
    ...
    await expect(downloadPage.locator('h1', { hasText: 'Download files' })).toBeVisible({ timeout: 10000 });
  ...
  test('upload file with password and download successfully', async ({ page, context }) => {
    ...
    await expect(downloadPage.locator('h2', { hasText: 'Unlock your files' })).toBeVisible();
    
    await downloadPage.locator('input[data-test="password"]').fill(testPassword);
  ...
  test('upload fails with invalid password on download', async ({ page, context }) => {
    const testPassword = 'SecurePassword123!';
    ...
    await downloadPage.locator('input[data-test="password"]').fill(wrongPassword);
    await downloadPage.locator('button[type="submit"]').click();
    
    await expect(downloadPage.locator('[data-test="error-message"]', { hasText: 'Invalid password' })).toBeVisible({ timeout: 5000 });
  });
  ...
  test('upload multiple files and download all', async ({ page, context }) => {
    await page.goto('/upload');
    ...
    const files = [
      ...
      { name: 'file3.txt', mimeType: 'text/plain', buffer: Buffer.from('Content 3') }
    ];
